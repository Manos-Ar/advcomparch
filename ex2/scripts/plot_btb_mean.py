            ...
            while line:
                ...
                if line.startswith("BTB"):
                    ...
                    while(len(tokens) > 1):
                        ...
                        if first:
                            total[predictor] = ([direction_mpki], [target_mpki])
                        else:
                            total[predictor][0].append(direction_mpki)
                            total[predictor][1].append(target_mpki)
                        line = fp.readline()
                        tokens = line.split(':')
                line = fp.readline()
            first = False
...
for key, (val1, val2) in total.items():
    print(key, val1, val2)
    val1 = gmean(val1)
    val2 = gmean(val2)
    print(key, val1, val2)
    x_axis.append(key)
    direction_mpki_axis.append(val1)
    target_mpki_axis.append(val2)
    print(x_axis)
    print(direction_mpki_axis)
