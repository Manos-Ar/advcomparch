...
# with os.scandir(sys.argv[2]) as it: #static
# 	Benchmarks = []
# 	for entry in it:
# 		if entry.is_file():
# 			# print(entry.name)
...
# print(Benchmarks)
#
with os.scandir(sys.argv[2]) as it: #2b
	Benchmarks = []
	for entry in it:
		if entry.is_file():
			# print(entry.name)
...
#
with os.scandir(sys.argv[3]) as it: #ras
	Benchmarks = []
	for entry in it:
		if entry.is_file():
			# print(entry.name)
...
with os.scandir(sys.argv[4]) as it: #local
	Benchmarks = []
	for entry in it:
		if entry.is_file():
			# print(entry.name)
...
with os.scandir(sys.argv[5]) as it: #tour
	Benchmarks = []
	for entry in it:
		if entry.is_file():
			# print(entry.name)
...
for bench in range(len(Benchmarks)):
	...
	for predictor in x_axis:
		mpki_axis_bench.append(mpki_axis[predictor][benchmark])
	...
	ax1.set_ylim(min(mpki_axis_bench) - 0.005, max(mpki_axis_bench) + 0.005)
	ax1.set_ylabel("$MPKI$")
	line1 = ax1.plot(mpki_axis_bench, label="mpki", color="green",marker='o')
