...
#define n_bits 2 
#define l_bits 2 /* last l_bits of PC used */
#define p_bits 0 /* length of bhr */
...
#define initial_address 0x00448404  /*PC of the first command in the given code*/
#define num_of_cmds 16 /*number of commands in the given code*/


#define paris 1 /*number of parallel issues/commits supported */
...
int RScapacity[rs_num]={2,  2,  2,  1,  3,  1};
...
////////////           mem int add mld
int FUcapacity[fu_num]={1,  2,  3,  1};
int FUcycles[fu_num]=  {1,  1,  3,  5};
bool FUpipelined[fu_num]= {true, false, false, false};

int miss_penalty=4; /*cache miss penalty in cycles*/
int rob_capacity=8;
    ...
    while(cur_cmd<num_of_cmds||!(qspec_end.empty())){ 
        ...
        for(int k=0; k<paris; k++){ /*parallel issue */
            ...
            for(int i=tm+1; ; i++){
                ...
                int b=a+FUcycles[fumap[ctype]]+pen-1; /// penalty
                if(i==1000)break; //pipaaa
                //cout<<"cur_cmd="<<cur_cmd<<'\n';
                //cout<<"a="<<a<<'\n';
                //cout<<"FUcycles[fumap[ctype]]="<<FUcycles[fumap[ctype]]<<'\n';
                //cout<<"b="<<b<<'\n';
                for(int j=a; j<=b; j++){
            ...
            if(!(cmdlist.empty())){
                //cout<<"FUcycles[fumap[ctype]]+pen="<<FUcycles[fumap[ctype]]+pen<<'\n';
                //cout<<"pen="<<pen<<'\n';
                //cout<<"cur_cmd="<<cur_cmd<<'\n';
            ...
            // cout<<"ex_time="<<ex_time<<",FUcycles="<<FUcycles[fumap[ctype]]<<",pen="<<pen<<'\n';
            ...
            //RS[10] FU[10] RScapacity FUcapacity FUcycles rsmap[com_num] fumap[com_num]
            ...
            if((commands[cur_cmd][0]==9||commands[cur_cmd][0]==10)&&isb){
                ...
                //cout<<"\nR[commands[cur_cmd][1]]="<<R[commands[cur_cmd][1]]<<'\n';
                //cout<<"outcome="<<outcm<<"\n\n";
                if(outcm) cmmnt+="outcome = T, ";
                else cmmnt+="outcome = NT, ";
                //cout<<"address="<<commands[cur_cmd][4]<<'\n';
                //cout<<"BHR="<<BHR<<'\n';
                //cout<<"PHT="<<'\n';
            ...
            //oxi panta!!!!!1 mono otan ginetai IS
            //de xreiazetai na perimenw to wr gt ta raw epiluontai me stalls
            if(isb){
            ...
            if(itt!=qspec_end.end()){
                //cout<<"tmpsofa="<<tm<<'\n';
            //if(tm==qspec_end.front()){ 
                //an tm == me kapoio stoixeio tou qspec_end, tote :
                // an problepse swsta apla to bgazeis apo thn oura
                //an provlepse lathos petas auto kai ta epomena opws kai sto versionpoutsa
                ...
                //bool outcome=bnez(commands[br_address][1]);
                ...
                //cout<<"outcome="<<outcome<<" ,prediction="<<predict(br_address)<<'\n';
                //cout<<"br_address="<<br_address<<'\n';
                bool mispred=false;
                if(outcome!=predict(commands[br_address][4])){
                    ...
                    //
                //     cout<<"     list of operations when misprediction was detected: \n";
                //     cout<<"         OP,IS, EX, WR,CMT\n";
                //     for (int ip = 0; ip < cmdlist.size(); ip++) {
                // 	    cout<<"         "<< cmdlist.at(ip).op <<", "<< cmdlist.at(ip).IS <<", ";
                // 	    cout<< cmdlist.at(ip).EXs <<"-"<< cmdlist.at(ip).EXf <<", "<< cmdlist.at(ip).WR  <<", "<< cmdlist.at(ip).CMT<< '\n';
                // 	}
                ...
                //cout<<"br_address = "<<br_address<<'\n';
                //cout<<commands[br_address][2]<<'\n';
                //cout<<"qbr_address empty?"<<qbr_address.empty()<<'\n';
                //cout<<"distance(qspec_end.begin(),itt)="<<distance(qspec_end.begin(),itt)<<'\n';
                ...
                //tha mporouses na kaneis clear tous pointers....version2
                //qrcp.erase(qrcp.begin()+itt-qspec_end.begin()); 
