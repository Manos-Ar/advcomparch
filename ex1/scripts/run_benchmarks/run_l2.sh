...
CONFS="512_8_64 512_8_128 512_8_256
...
for conf in $CONFS; do
	...
	pin_cmd="$PIN_EXE -t $PIN_TOOL -o $outFile -L2c ${L2size} -L2a ${L2assoc} -L2b ${L2bsize} -L1c ${L1size} -L1a ${L1assoc} -L1b ${L1bsize} -TLBe ${TLBe} -TLBp ${TLBp} -TLBa ${TLBa} -L2prf ${L2prf} -- $cmd"
	time $pin_cmd
done
done
